export const photos = [
    {
        key:21,
        path:'/gallery (1).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Tile Floors'
    },
    {
        key:22,
        path:'/gallery (2).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Tiled Showers'
    },
    {
        key:23,
        path:'/gallery (4).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Backsplashes'
    },
    {
        key:24,
        path:'/gallery (6).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Tiled Showers'
    },
    {
        key:25,
        path:'/gallery (8).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Tiled Showers'
    },
    {
        key:26,
        path:'/gallery (9).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Tiled Showers'
    },
    {
        key:27,
        path:'/gallery (10).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Tiled Showers'
    },
    {
        key:28,
        path:'/gallery (11).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Remodels'
    },
    {
        key:29,
        path:'/gallery (12).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Remodels'
    },
    {
        key:30,
        path:'/gallery (13).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Remodels'
    },
    {
        key:31,
        path:'/gallery (14).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Fireplace Surrounds'
    },
    {
        key:32,
        path:'/gallery (15).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Tile Floors'
    },
    {
        key:33,
        path:'/gallery (16).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Tile Floors'
    },
    {
        key:34,
        path:'/gallery (18).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Remodels'
    },
    {
        key:35,
        path:'/gallery (19).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Tile Floors'
    },
    {
        key:36,
        path:'/gallery (20).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Tile Floors'
    },
    {
        key:37,
        path:'/gallery (21).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Tile Floors'
    },
    {
        key:38,
        path:'/gallery (22).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Tile Floors'
    },
   
    {
        key:39,
        path:'/gallery (23).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Backsplashes'
    },
    {
        key:40,
        path:'/gallery (24).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Tiled Showers'
    },
    {
        key:41,
        path:'/gallery (25).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Remodels'
    },
    {
        key:42,
        path:'/gallery (27).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Remodels'
    },
   
    {
        key:45,
        path:'/gallery (28).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Remodels'
    },
    {
        key:46,
        path:'/gallery (29).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Tiled Showers'
    },
    {
        key:47,
        path:'/gallery (30).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Remodels'
    },
    {
        key:48,
        path:'/gallery (31).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Remodels'
    },
    {
        key:49,
        path:'/gallery (32).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Tiled Showers'
    },
    {
        key:50,
        path:'/gallery (33).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Tiled Showers'
    },
    {
        key:51,
        path:'/gallery (34).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Tiled Showers'
    },
    {
        key:52,
        path:'/gallery (36).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Tiled Showers'
    },
    {
        key:53,
        path:'/gallery (37).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Remodels'
    },
    {
        key:54,
        path:'/gallery (38).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Tile Floors'
    },
    {
        key:55,
        path:'/gallery (39).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Tiled Showers'
    },
    {
        key:56,
        path:'/gallery (40).JPEG',
        alt:'BJs painting Saint George Utah project',
        description:'Tiled Showers'
    },
    
];